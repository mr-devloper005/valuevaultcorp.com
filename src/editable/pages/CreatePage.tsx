        ...
        <main className="px-4 py-16 sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] border border-[var(--editable-border)] bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2b67b5]">{pagesContent.create.locked.badge}</p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 bg-[#ff2b2b] px-6 py-3 text-sm font-black uppercase tracking-[0.06em] text-white">Login <ArrowRight className="h-4 w-4" /></Link>
      ...
      <main>
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.8rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2b67b5]">{pagesContent.create.hero.badge}</p>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-transparent bg-[var(--slot4-page-text)] text-white' : 'border-[var(--editable-border)] bg-[#f7f9ff] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-[var(--editable-border)] bg-[#f7f9ff] p-5 sm:p-7">
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#ff2b2b] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5">
