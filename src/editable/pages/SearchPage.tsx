...
const getImage = (post: SitePost) => {
  ...
  return media || compactRaw(content.featuredImage) || compactRaw(content.image) || compactRaw(content.thumbnail) || images || '/placeholder.svg?height=800&width=1200'
}
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      <div className={`relative overflow-hidden bg-[var(--slot4-media-bg)] ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
        <img src={image} alt="" className="h-full w-full object-cover opacity-92 transition duration-500 group-hover:scale-105" />
      ...
      <div className="p-5 sm:p-6">
        <h2 className="line-clamp-3 text-2xl font-black leading-[0.95] tracking-[-0.06em] text-[var(--slot4-page-text)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main>
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.82fr_1.18fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2b67b5]">{pagesContent.search.hero.badge}</p>
