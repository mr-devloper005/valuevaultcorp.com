...
const inputClass = 'h-12 rounded-2xl border border-[var(--editable-border)] bg-[#f7f9ff] px-4 text-base font-bold text-current outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[#ff2b2b] px-6 text-sm font-black uppercase tracking-[0.18em] text-white shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalSignupForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <input
        className={`${inputClass} text-black placeholder:text-black/35`}
